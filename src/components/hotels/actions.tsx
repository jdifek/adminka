"use client";

import { useState } from "react";
import { Button } from "@/components/ui/button";
import { Pencil, Trash2 } from "lucide-react";
import { DeleteDialog } from "@/components/delete-dialog";
import { useToast } from "@/components/ui/use-toast";
import { useSWRConfig } from "swr";
import { Hotels } from "./columns";
import { HotelDialog } from "./hotel-dialog";

interface HotelsActionsProps {
  hotels: Hotels;
}

export function HotelsActions({ hotels }: HotelsActionsProps) {
  const [isEditOpen, setIsEditOpen] = useState(false);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const { toast } = useToast();
  const { mutate } = useSWRConfig();

  const handleDelete = async () => {
    try {
      await fetch(`/api/hotels/${hotels.id}`, {
        method: "DELETE",
      });
      toast({
        title: "Hotel deleted",
        description: "The hotel has been successfully deleted.",
      });
      mutate("/api/hotels");
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to delete the hotel.",
        variant: "destructive",
      });
    }
    setIsDeleteOpen(false);
  };

  return (
    <>
      <div className="flex items-center justify-end gap-2">
        <Button variant="ghost" size="icon" onClick={() => setIsEditOpen(true)}>
          <Pencil className="h-4 w-4" />
        </Button>
        <Button variant="ghost" size="icon" onClick={() => setIsDeleteOpen(true)}>
          <Trash2 className="h-4 w-4" />
        </Button>
      </div>

      <HotelDialog // Обновлено на правильный компонент
        open={isEditOpen}
        onOpenChange={setIsEditOpen}
        hotel={hotels} // Передаем данные отеля
        onClose={() => {
          setIsEditOpen(false);
          mutate("/api/hotels");
        }}
      />

      <DeleteDialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen} onConfirm={handleDelete} />
    </>
  );
}
