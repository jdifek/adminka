...
function ContractsPage() {
  const { data: contracts, error, mutate } = useSWR("/api/contracts", fetcher);
  ...
  const filteredlast = filteredContracts.filter((el) => el.status !== "COMPLETED");
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable columns={columns} data={filteredlast} />

      <ContractDialog
        mutate={mutate} // Передаем функцию mutate
        open={isDialogOpen}
...
export default withRole(ContractsPage, "owner");
